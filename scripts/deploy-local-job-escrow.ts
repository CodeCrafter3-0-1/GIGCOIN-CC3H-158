...
async function main() {
  ...
  const escrow = await JobEscrow.deploy(tokenAddress, deployer.address);
  ...
  let nextEnvText = upsertEnvValue(envText, "ESCROW_ADDRESS", escrowAddress);
  nextEnvText = upsertEnvValue(nextEnvText, "RPC_URL", "http://127.0.0.1:8545");
  writeFileSync(backendEnvPath, nextEnvText);

  console.log("Updated backend/.env with the local escrow address.");
  ...
  console.log("Updated frontend/.env with the local escrow address.");
